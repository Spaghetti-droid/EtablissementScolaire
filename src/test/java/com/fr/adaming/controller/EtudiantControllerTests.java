...
public class EtudiantControllerTests {
	...
	public void testCreatingEtudiantWithNullEmail_shouldReturnNull() throws Exception { 
		...
		assertThat(responseAsString).isNull();
	...
	public void testReadByExistingId_ShouldReturnTrue() throws Exception{
		...
		EtudiantUpdateDto respExam = mapper.readValue(respBodyString, EtudiantUpdateDto.class);

		ClasseUpdateDto expectedClasse = new ClasseUpdateDto();
		expectedClasse.setId(1);
		expectedClasse.setName("Terminal");

		EtudiantUpdateDto expectedEtu = new EtudiantUpdateDto(2, "martinez", "Lucie", "25 fdd", 69500, "Bron", Sexe.F, 36, 0656321425, "martinez@lucie.com", expectedClasse);

		assertThat(respDto).isNotNull();
		assertThat(respDto).hasFieldOrPropertyWithValue("message", "SUCCESS");
		assertEquals(expectedEtu, respExam);
